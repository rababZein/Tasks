 ...
 *      - If the command type is 0 (create token), add the token ID and its expiry time (current time $T plus $expiryLimit) to the $tokens array.
 *      - If the command type is 1 (reset token), check if the token ID exists in the $tokens array and if the current time $T is less than or equal to the existing expiry time. 
 *            If so, we update the expiry time to the current time $T plus $expiryLimit.
